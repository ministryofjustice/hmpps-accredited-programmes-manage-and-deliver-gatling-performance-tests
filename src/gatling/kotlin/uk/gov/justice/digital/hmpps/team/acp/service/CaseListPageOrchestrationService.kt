...
import uk.gov.justice.digital.hmpps.team.acp.constants.CHANGE_COHORT_GENERAL_OFFENCE
import uk.gov.justice.digital.hmpps.team.acp.constants.LDC_FLAG_MAY_NEED_AN_LDC_ADAPTED_PROGRAMME
...
class CaseListPageOrchestrationService(
    ...
    fun getReferralDetailsPageAndDoChecks() =
        HttpDsl
            .http("GET - Referral details Page")
            .get { session ->
    ...
    fun getProgrammeNeedsIdentifierPageAndDoChecks() =
        HttpDsl
            .http("GET - Programme needs identifier Page")
            .get { session ->
                val referralId = session.getString(CaseListSimulationSession.REFERRAL_ID.sessionKey)
    ...
    fun getAvailabilityAndMotivationPageAndDoChecks() =
        HttpDsl
            .http("GET - Availability and motivation Page")
            .get { session ->
                val referralId = session.getString(CaseListSimulationSession.REFERRAL_ID.sessionKey)
    ...
    fun getAttendanceHistoryPageAndDoChecks() =
        HttpDsl
            .http("GET - Attendance history Page")
            .get { session ->
                val referralId = session.getString(CaseListSimulationSession.REFERRAL_ID.sessionKey)
    ...
    fun getUpdateLearningDisabilitiesAndChallengesPageAndDoChecks() =
        HttpDsl
            .http("GET - Update learning disabilities and challenges (LDC) status Page")
            .get { session ->
                val referralId = session.getString(CaseListSimulationSession.REFERRAL_ID.sessionKey)
    ...
    fun postUpdateLearningDisabilitiesAndChallengesPageAndDoChecks() =
        HttpDsl
            .http("POST - Update learning disabilities and challenges (LDC) status Page")
            .post { session ->
                val referralId = session.getString(CaseListSimulationSession.REFERRAL_ID.sessionKey)
                "/referral/$referralId/update-learning-disabilities-and-challenges"
    ...
    fun getReferralDetailsWithLdcUpdatedPageAndDoChecks() =
        HttpDsl
            .http("GET - Referral details when LDC updated Page")
            .get { session ->
                val referralId = session.getString(CaseListSimulationSession.REFERRAL_ID.sessionKey)
    ...
    fun getReferralDetailsWithCohortUpdatedPageAndDoChecks() =
        HttpDsl
            .http("GET - Referral details when cohort updated Page")
            .get { session ->
                val referralId = session.getString(CaseListSimulationSession.REFERRAL_ID.sessionKey)
    ...
    fun getUpdateReferralStatusPageAndDoChecks() =
        HttpDsl
            .http("GET - Update Referral Status Page")
            .get { session ->
                val referralId = session.getString(CaseListSimulationSession.REFERRAL_ID.sessionKey)
