...
import uk.gov.justice.digital.hmpps.team.acp.jdbc.GroupSessionsAndAttendanceFeeder
...
class GroupSessionsAndAttendanceScenarioService(
    private val groupSessionsAndAttendanceFeeder: GroupSessionsAndAttendanceFeeder = GroupSessionsAndAttendanceFeeder(),
    private val httpRequestHelper: HttpRequestHelper = HttpRequestHelper(),
    private val signInService: SignInService = SignInService(),
    private val pageOrchestrationService: GroupSessionsAndAttendancePageOrchestrationService = GroupSessionsAndAttendancePageOrchestrationService(),
) {
    fun buildScenario(
        scenarioName: String,
        pauses: GroupSessionsAndAttendancePauseConfig,
        ...
        val singleGroupDetailsJourney =
            CoreDsl
                .feed(groupSessionsAndAttendanceFeeder.getJdbcFeederForGroupSessionsAndAttendance())
                .pause(pauses.beforeStart.first, pauses.beforeStart.second)
                .exec(pageOrchestrationService.getGroupSessionsAndAttendancePageAndDoChecks())
                .pause(pauses.onGroupSessionsAndAttendancePage.first, pauses.onGroupSessionsAndAttendancePage.second)
                // Schedule a pre-group one-to-one catch-up session
                .exec(pageOrchestrationService.getSchedulePreGroupOneToOneSessionTypePageAndDoChecks())
                .pause(pauses.onSchedulePreGroupOneToOneSessionTypePage.first, pauses.onSchedulePreGroupOneToOneSessionTypePage.second)
